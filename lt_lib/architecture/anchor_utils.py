...
class AnchorGenerator(nn.Module):
    ...
    def __init__(
        ...
        if type(scales[0]) is float or int:
            self.cell_anchors = [
                self.generate_anchors(tuple(size * scale for scale in scales), aspect_ratios) for size in sizes
            ]
        elif type(scales[0]) is tuple or list:
            self.cell_anchors = [
                self.generate_anchors(tuple(size * scale for scale in scales_at_pyr_lvl), aspect_ratios)
                for size, scales_at_pyr_lvl in zip(sizes, scales)
            ]
        else:
    ...
    def num_anchors_per_location_per_pyramid_level(self) -> List[int]:
        ...
        if type(self.scales[0]) is float or int:
            ...
            return [
                len(scales_at_pyr_lvl) * len(self.aspect_ratios)
    ...
    def forward(self, image_list: Tensor, feature_maps: List[Tensor]) -> List[Tensor]:
        # Modified because decided to not use the GeneralizedRCNNTransform and so did not use the ImageList type
